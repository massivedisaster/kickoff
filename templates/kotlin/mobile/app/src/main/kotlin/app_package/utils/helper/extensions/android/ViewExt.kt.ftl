 ...
 *
 * @param viewGroupScroller ViewGroup that support scrolling (nestedscrollview, scrollview, etc)
 */
...
fun View.onClickDebounce(bouncer: DebounceTimer, milWait: Long = DebounceTimer.DEFAULT_DELAY, action: () -> Unit) {
    onClick { bouncer.debounceRunFirst(milWait) { action() } }
}
...
fun View.convertToBitmap(): Bitmap {
    val measureSpec = MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED)
