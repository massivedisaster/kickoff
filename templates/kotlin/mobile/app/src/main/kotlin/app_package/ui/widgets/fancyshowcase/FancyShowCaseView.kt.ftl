...
import ${configs.packageName}.ui.widgets.fancyshowcase.listener.AnimationListener
import ${configs.packageName}.ui.widgets.fancyshowcase.listener.DismissListener
import ${configs.packageName}.ui.widgets.fancyshowcase.listener.OnQueueListener
import ${configs.packageName}.ui.widgets.fancyshowcase.listener.OnViewInflateListener
...
class FancyShowCaseView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, @AttrRes defStyleAttr: Int = 0)
    ...
    private fun addFancyImageView() {
        FancyImageView(activity).apply {
            ...
            if (mFocusBorderSize > 0) {
                setBorderParameters(mFocusBorderSize)
            }
                ...
                object : ViewTreeObserver.OnGlobalLayoutListener {
                    override fun onGlobalLayout() {
                        ...
                        ViewAnimationUtils.createCircularReveal(this@FancyShowCaseView,
                            ...
                            interpolator = AnimationUtils.loadInterpolator(activity, android.R.interpolator.accelerate_cubic)
                            start()
                        }
                    }
                })
    ...
    private fun doCircularExitAnimation() {
        ...
        val revealRadius = hypot(width.toDouble(), height.toDouble()).toInt()
        ViewAnimationUtils.createCircularReveal(this, mCenterX, mCenterY, revealRadius.toFloat(), 0f).apply {
            duration = mAnimationDuration.toLong()
            interpolator = AnimationUtils.loadInterpolator(activity, android.R.interpolator.decelerate_cubic)
            addListener(object : AnimatorListenerAdapter() {
                override fun onAnimationEnd(animation: Animator) {
                    removeView()
                    mAnimationListener?.onExitAnimationEnd()
    ...
    fun isShownBefore() = id != null && isShownBefore(context, id!!)
    ...
    private fun shouldShowCircularAnimation() = Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP
    ...
    class Builder(private val activity: Activity) {
         ...
         */
        fun build() = FancyShowCaseView(activity, focusedView, clickableView, mId, mTitle, mSpannedTitle, mTitleGravity, mTitleStyle, mTitleSize, mTitleSizeUnit,
                    focusCircleRadiusFactor, mBackgroundColor, mFocusBorderSize, mCustomViewRes, viewInflateListener,
                    ...
                    mFocusAnimationMaxValue, mFocusAnimationStep, delay, autoPosText, mMargin)
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun resetShowOnce(context: Context, id: String) = preferences(context).edit().remove(id).apply()
         ...
         */
        @JvmStatic
        fun resetAllShowOnce(context: Context) = preferences(context).edit().clear().apply()
