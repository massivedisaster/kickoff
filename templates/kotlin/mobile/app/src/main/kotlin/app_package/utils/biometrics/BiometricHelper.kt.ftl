...
class BiometricHelper(private val fragment: Fragment) {
     ...
     */
    fun getBiometricType() = if (cryptoHelper.checkOneBiometricMustBeEnrolled()) {
        when (biometricManager.canAuthenticate(BiometricManager.Authenticators.BIOMETRIC_WEAK)) {
            BiometricManager.BIOMETRIC_SUCCESS, BiometricManager.BIOMETRIC_ERROR_NONE_ENROLLED -> when {
     ...
     * @return [Boolean]
     */
    fun biometricEnable() = getBiometricType() != BiometricType.NONE
    ...
    fun showBiometricPrompt(
        ...
        showBiometricPrompt(biometricPromptInfo.toPromptInfo(), getAuthenticationCallback(onSuccess, onError, onFailed))
    }
    ...
    fun showBiometricPrompt(
        ...
        showBiometricPrompt(biometricPromptInfo.toPromptInfo(), getAuthenticationCallback(onSuccess, onError, onFailed), crypto)
    }
    ...
    fun showBiometricPrompt(
        ...
        if (biometricEnable()) {
            BiometricPrompt(fragment, ContextCompat.getMainExecutor(context), authenticationCallback).apply {
