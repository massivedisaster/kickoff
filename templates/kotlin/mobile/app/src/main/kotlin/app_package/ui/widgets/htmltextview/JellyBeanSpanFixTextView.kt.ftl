...
/**
 * A [AppCompatTextView] that insert spaces around its text spans where needed to prevent
 ...
 */
open class JellyBeanSpanFixTextView : AppCompatTextView {
    ...
    private fun fixOnMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (text is Spanned) {
            ...
            Timber.d("The text isn't a Spanned")
     ...
     */
    private fun fixSpannedWithSpaces(builder: SpannableStringBuilder, widthMeasureSpec: Int, heightMeasureSpec: Int) {
        val startFix = System.currentTimeMillis()
        val result = addSpacesAroundSpansUntilFixed(builder, widthMeasureSpec, heightMeasureSpec)
        ...
        Timber.d("fixSpannedWithSpaces() duration in ms: $fixDuration")
    }

    private fun addSpacesAroundSpansUntilFixed(builder: SpannableStringBuilder, widthMeasureSpec: Int, heightMeasureSpec: Int): FixingResult {
        val spans = builder.getSpans<Any>(0, builder.length, Any::class.java)
        val spansWithSpacesBefore = ArrayList<Any>(spans.size)
        val spansWithSpacesAfter = ArrayList<Any>(spans.size)
        ...
        Timber.d("Could not fix the Spanned by adding spaces around spans")
        return FixingResult.notFixed()
    }

    private fun isNotSpace(text: CharSequence, where: Int) = where < 0 || where >= text.length || text[where] != ' '
    ...
    @SuppressLint("WrongCall")
    private fun removeUnneededSpaces(widthMeasureSpec: Int, heightMeasureSpec: Int, builder: SpannableStringBuilder, result: FixingResult) {
    ...
    private fun fallbackToString(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        Timber.d("Fallback to unspanned text")
        val fallbackText = text.toString()
        setTextAndMeasure(fallbackText, widthMeasureSpec, heightMeasureSpec)
    }

    private class FixingResult private constructor(val fixed: Boolean, val spansWithSpacesBefore: List<Any>?, val spansWithSpacesAfter: List<Any>?) {
        companion object {
            fun fixed(spansWithSpacesBefore: List<Any>, spansWithSpacesAfter: List<Any>) = FixingResult(true, spansWithSpacesBefore, spansWithSpacesAfter)
            fun notFixed() = FixingResult(false, null, null)
        }
    }
